<template>
  <div>
    <div class="list-content">
      ...
      <div class="choosed"> 服务运行统计>{{value}}</div><!-- 显示对应选择 -->
     </div>  
     <!-- 类别及产品列表 -->
  ...
  methods: {
    //----------------初始化模块--------------------------------
        ...
        businessdefinePort: function() {
          this.$http
            .get(
              "http://report.ruiyunnet.com/report/report/businessdefine?id=" + 1
            )
            .then(
              response => {
        ...
        handleCurrentChange(val) {
          ...
          */
          this.$nextTick(function(){
            if(this.isSearching==true&this.isSort==true){
              ...
              this.echartClear();
              this.toggleSelection([this.noTagPageTableData[0]]);//对点击搜索后Echart进行设置
            }
          }
          ) 
          this.loading=false;
    ...
    // ------------------点击各类按钮响应方法--------------------
      ...
      //---localstorage---------
        prodctionSetItem:function(){
            localStorage.setItem("nav2所选产品",this.value);
        },
        productionGetItem:function(){
          this.value=localStorage.getItem("nav2所选产品");//从缓存获取this.value
          if(this.value==null){
            this.value="掌中宝商品"
          } //如果没有 则赋值；
        },
